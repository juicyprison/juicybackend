...
router.post("/generate-code", (req, res) => {
  ...
  linkCodes.set(code, { minecraftUuid, minecraftName, expiresAt: Date.now() + CODE_EXPIRY_MS, token });
  linkTokens.set(token, { minecraftUuid, minecraftName, expiresAt: Date.now() + TOKEN_EXPIRY_MS });

  console.log(`[Link] Kode ${code} genereret for ${minecraftName}`);
  res.json({ code, token });
});
